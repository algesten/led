...
fn row_col_to_char(doc: &dyn Doc, row: usize, col: usize) -> CharOffset {
    let row = row.min(doc.line_count().saturating_sub(1));
    let len = doc.line_len(Row(row));
    let col = col.min(len);
    CharOffset(doc.line_to_char(Row(row)).0 + col)
}
...
pub fn apply_indent(buf: &mut BufferState, row: usize, new_indent: &str, adjust_cursor: bool) {
    ...
    let line_start = buf.doc().line_to_char(Row(row));
    if old_len > 0 {
        buf.remove_text(line_start, CharOffset(line_start.0 + old_len));
    }
...
pub fn kill_region(buf: &mut BufferState) -> Option<(String, usize, usize, usize)> {
    ...
    let col = start.0 - buf.doc().line_to_char(row).0;
    Some((killed, row.0, col, col))
}
