...
pub fn adjust_scroll(buf: &BufferState, dims: &Dimensions) -> (usize, usize) {
    ...
    let scroll_vl = visual_line_count(buf.doc().line_display_width(Row(sr)), text_width);
    if ssl >= scroll_vl {
...
fn compute_cursor_vrow(
    ...
    let scroll_vl = visual_line_count(doc.line_display_width(Row(sr)), text_width);
    vrow += scroll_vl - ssl;

    // Intermediate lines
    for li in (sr + 1)..cursor_row {
        vrow += visual_line_count(doc.line_display_width(Row(li)), text_width);
        if vrow > 10000 {
...
fn scroll_to_place_cursor_at_vrow(
    ...
    for li in (0..cursor_row).rev() {
        ...
        let vl = visual_line_count(doc.line_display_width(Row(li)), text_width);
        if vl <= remaining {
            remaining -= vl;
            new_scroll = li;
            new_sub = 0;
...
fn cursor_visual_position(
    ...
    led_core::with_line_buf(|line_buf| {
        cursor_visual_position_inner(doc, cursor_row, cursor_col, text_width, line_buf)
    })
}

fn cursor_visual_position_inner(
    doc: &dyn Doc,
    cursor_row: usize,
    cursor_col: usize,
    text_width: usize,
    ...
    doc.line(Row(cursor_row), line_buf);
    let (cursor_display, cursor_cm) = expand_tabs(&line_buf);
...
fn visual_col_of(doc: &dyn Doc, row: usize, col: usize, text_width: usize) -> usize {
    led_core::with_line_buf(|line_buf| visual_col_of_inner(doc, row, col, text_width, line_buf))
}

fn visual_col_of_inner(
    doc: &dyn Doc,
    row: usize,
    col: usize,
    text_width: usize,
...
fn compute_move_up(
    ...
    led_core::with_line_buf(|line_buf| {
        ...
        if sub > 0 {
            ...
            doc.line(Row(new_row), line_buf);
            let (prev_display, prev_cm) = expand_tabs(line_buf);
...
fn compute_move_down(
    ...
    led_core::with_line_buf(|line_buf| {
        ...
        if sub + 1 < chunks.len() {
            ...
            let new_row = cursor_row + 1;
            doc.line(Row(new_row), line_buf);
            let (next_display, next_cm) = expand_tabs(line_buf);
