...
- **Macro replay does not dispatch editor cursor moves** (surfaced during M22 golden review). Legacy's `handle_action` (`led/src/model/action/mod.rs:108-127`) only has `Action::MoveDown` / `Action::LineEnd` / `Action::PageUp` / etc. arms for the **browser-side** code path; editor-side cursor movement is stream-handled in `derived.rs`. When `KbdMacroExecute` recursively calls `handle_action(state, MoveDown)` during macro replay, the recursive call falls through `_ => {}` for editor focus and the cursor doesn't move. Recorded text edits (`InsertChar`, etc.) DO replay because their handlers are imperative; pure cursor moves silently drop. The rewrite correctly replays cursor moves through the same `Dispatcher::run_command` path, so the M22 goldens for `keybindings/kbd_macro/e_replay`, `keybindings/ctrl_x/e`, `features/macros/record_and_replay`, `features/macros/record_insert_play_twice` were refreshed against the rewrite (cursor positions now reflect the actual replay). Per `ROADMAP.md` § "Golden-review discipline" the right fix is on `main`: route legacy's macro replay through the same path that processes a real keystroke (so the FRP stream sees it). Non-trivial in legacy's combinator graph; worth doing only if someone actually uses macros on `main` between now and rewrite cutover.
