...
- **`clippy::too_many_arguments` is fixed by bundling, never by `#[allow]`.** When a function reaches the lint threshold (>7 args), introduce a narrow argument-bundle struct holding *only* the references that function reads/writes — and convert callers to build the bundle. The struct is named after the operation (`FlushUndoArgs`, `LspGotoApply`, `NavCtx`, `LspEditApply`, `RenderContentArgs`, `FileSearchHeaderPaint`), not after the data category. Two natural shapes: (a) for stateless helpers, a plain struct passed by value or by reference (`fn flush_undo(conn: &Connection, args: &FlushUndoArgs<'_>)`); (b) for stateful operations that need a receiver, an `impl<'a> Bundle<'a> { fn op(&mut self, …) }` method (`LspGotoApply::apply`, `NavCtx::next_issue`). The runtime's existing `Dispatcher<'a>` is the canonical example — `Dispatcher::dispatch_key` and `Dispatcher::run_command` started as 24-arg free functions and became methods. **Do not** introduce a shared mega-struct that bundles unrelated state across multiple call sites; each site gets its own narrow bundle. The only sanctioned use of `#[allow(clippy::too_many_arguments)]` is inside test helpers where every state slot is genuinely caller-controlled and the test would otherwise lose readability — and even then the helper signature should be the smallest possible (drop unused args before reaching for the allow). When in doubt, look at the existing bundle precedents in `runtime/src/dispatch/mod.rs` (`Dispatcher`), `runtime/src/lib.rs` (`LspEditApply`, `LspGotoApply`), `runtime/src/dispatch/nav.rs` (`NavCtx`).
