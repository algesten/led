...
- **Push server sends publishDiagnostics mid-session after pull mode was detected**: pull remains primary. The push is dropped if a pull is currently in flight for that path (`DiagPushResult::Drop`); otherwise it lands in the fallback cache and surfaces as `LspEvent::PushFallback`, which the runtime fold accepts only while no pull has answered (per `LspState.pull_state`). The mode never flips at runtime — per audit Theme L Target C, the project rule "Pull diagnostics only" forbids the legacy permanent pull→push downgrade.
