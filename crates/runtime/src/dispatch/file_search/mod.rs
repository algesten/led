...
use nav::{handle_enter, move_selection};
use replace_all::{advance_to_next_pending, apply_replace_all, ensure_replacements_len};
use replace_one::{replace_selected, unreplace_selected};
...
pub(super) fn clamp_scroll_to_selection(
    state: &mut FileSearchState,
    side_rows: usize,
) {
...
pub(super) fn preview_scroll_top(line: usize, body_rows: usize) -> usize {
    if body_rows == 0 {
        return line;
    }
